...
**Reason**: Idle detection hid mail the photographer wanted to choose per event and per channel.
**Migration**: Use “Photographer notices follow Settings channel preferences” and “Notifications are configured in Settings”. Existing `photographer_away` true maps to Email on for new booking and client feedback; false maps to Email off for those two rows.
